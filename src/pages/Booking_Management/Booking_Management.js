...
export default function Booking_Management(props) {
  ...
  const [modalState, setModalState] = useState({ isViewDetails: "" });
  ...
  const getCategoriesContent = async (
    ...
    try {
      const { data } = await axios.get(
        `/private/singleTaxiBookingList?page=${page}&limit=${limit}&Incomplete=${Incomplete}&search=${search}&status=${status}&serviceType=${serviceType}&start_date=${start_date}&end_date=${end_date}&rideTypeStatus=0`
      );
      console.log(data);
  ...
  useEffect(() => {
    if (state && state !== undefined) {
      ...
      getCategoriesContent(page, rowsPerPage, Incomplete, search, "ongoing", changeID);
      // console.log(state.page);
      // console.log(state.limit);
      // console.log(state.Incomplete);
      // console.log(state.search);
  ...
  const handleParentTabChange = (event, newValue) => {
    ...
    switch (event.target.innerText) {
      case "SINGLE TAXI":
        getCategoriesContent(page, limit, false, search, "ongoing", "62d53aa9bf652aa3778946ca", start, end);
      ...
      case "DESIGNATED DRIVER":
        ...
        break;
      case "PET SERVICE":
        getCategoriesContent(page, limit, false, search, "ongoing", "62d53ad1bf652aa3778946d0", start, end);
        ...
        break;
      case "PACKAGE SERVICE":
        getCategoriesContent(page, limit, false, search, "ongoing", "62d53ae6bf652aa3778946d3", start, end);
  ...
  return (
    <React.Fragment>
                      ...
                      <TableHead>
                        <TableRow>
                          ...
                          <TableCell style={{ textAlign: "center", fontWeight: "bold" }}>
                            ...
                            {statusString == "completed" ? `Date & Time(Start)` : ` Date & Time`}
                          </TableCell>
                         { statusString == "completed" ? <TableCell style={{ textAlign: "center", fontWeight: "bold" }}>
                          
                          {statusString == "completed" ? `Date & Time(End)` : ` Date & Time`}
                        </TableCell>:false}
                          ...
                          {statusString == "cancel"?false:<TableCell style={{ textAlign: "center", fontWeight: "bold" }}>Time(estimated)</TableCell>}
                          <TableCell style={{ textAlign: "center", fontWeight: "bold" }}>{statusString!="cancel"?"Total Fare($)":"Cancellation Fee($)"}</TableCell>
                          ...
                          {statusString == "completed" ? (
                            ...
                            false
                              ...
                              {statusString == "cancel"?<TableCell style={{ textAlign: "center", fontWeight: "bold" }}>Action</TableCell>:false}
                          {/* <TableCell
                            style={{ textAlign: "center", fontWeight: "bold" }}
                          >
                            Actions
                        ...
                        {tableData
                          ...
                          .map((category, index) => (
                            <TableRow hover key={index}>
                              ...
                              <TableCell className={classes.textMiddle} style={{ textAlign: "center" }}>
                                {category.status === "complete"
                                  ...
                                  // category?.bookingAcceptTime
                                    ? 
                                    // `${moment(category.bookingAcceptTime).format("Do MMM YY ,hh:mm a")}
                                  ...
                                  : 
                                  category?.createdAt
                                  // category?.bookingAcceptTime // for cancel and ongoing
                                  ? 
                                  `${moment(category.createdAt).format("Do MMM YY ,hh:mm a")}`
                                  // `${moment(category.bookingAcceptTime).format("Do MMM YY ,hh:mm a")}`
                                  : "N/A"}
                              </TableCell>
                              {category.status === "complete"?<TableCell className={classes.textMiddle} style={{ textAlign: "center" }}>
                              {moment(
                                        category.dropTime
                              ...
                              <TableCell className={classes.textMiddle} style={{ textAlign: "center" }}>
                                <span style={{cursor:"pointer"}} className="hyperLinkHoverColor" onClick={()=>viewDriver(category)}>{get(category, "driver.firstName", "N/A") + " " + get(category, "driver.lastName", "N/A")}</span>
                                <br />
                             ...
                             {(category?.status=="complete")||(category?.status=="cancel")? 
                              <TableCell   onClick={() => setLocationText(locationText === category._id ? null : category._id)}
                              style={{
                                 textAlign: "center",
                                whiteSpace: locationText === category._id ? "" : "nowrap",
                                maxWidth: "150px",
                              ...
                              {category?.status=="cancel"?false:<TableCell className={classes.textMiddle} style={{ textAlign: "center" }}>
                                {get(category, "estimateTime", "N/A")}
                              ...
                              {statusString == "ongoing" && changeID !== "62d53abebf652aa3778946cd" ? (
                                <TableCell className={classes.textMiddle} style={{ textAlign: "center" }}>
                                  ...
                                  >
                                    <Tooltip title={<span className="TooltipCustomSize">End Ride</span>} arrow>
                              ...
                              {statusString == "completed" ? (
                                <TableCell className={classes.textMiddle} style={{ textAlign: "center" }}>
                                  <Button
                                    onClick={() => {
                                      BookingDetails(category.bookingId);
                                    }}
                                    // size="small"
                                  ...
                                  >
                                    <Tooltip title={<span className="TooltipCustomSize">booking details</span>} arrow>
                                      <VisibilityIcon />
                                    </Tooltip>
                                  </Button>
                                </TableCell>
      ...
      <Modal
        maxWidth="lg"
        width="740px"
        isOpen={modalState.isViewDetails}
        onClose={() => {
          setModalState({
            isViewDetails: false,
          });
        ...
        content={
          <>
            <Formik
              initialValues={{
                rideAdminCommission: sanitizeDecimal(get(selectedViewDetail, "rideAdminCommission", 0)),
                driverBookingPrice: sanitizeDecimal(get(selectedViewDetail, "driverBookingPrice", 0)), //total driver
                driverPrice: sanitizeDecimal(get(selectedViewDetail, "driverPrice", 0)), //driver 1
                driverTip: sanitizeDecimal(get(selectedViewDetail, "driverTip", 0)),
                pairDriverPrice: sanitizeDecimal(get(selectedViewDetail, "pairDriverPrice", 0)), //driver 2
              }}
              ...
              {(formikBag) => (
                <Form style={{ width: "100%" }}>
                  <div className="container">
                    ...
                    {selectedViewDetail?.status == "complete" &&
                    selectedViewDetail?.serviceType !== "62d53abebf652aa3778946cd" ? (
                      <div className="row my-3">
                        <div className="col-md-6">
                          <label>
                            Admin Commission <span style={{ color: "#85BB65", fontWeight: "bold" }}>(in USD)</span>:
                          </label>
                          <Field name="rideAdminCommission">
                            {({ field }) => (
                              <Input
                                {...field}
                                type="number"
                        ...
                        <div className="col-md-6">
                          <label>
                            Driver Fare <span style={{ color: "#85BB65", fontWeight: "bold" }}>(in USD)</span>:
                          </label>
                          <Field name="driverBookingPrice">
                            {({ field }) => (
                              <Input
                                {...field}
                                type="number"
                                // onChange={(e) => {
                                //   formikBag.setFieldValue("rideAdminCommission", e.target.value);
                                // }}
                                error={formikBag.touched.tip && formikBag.errors.tip ? formikBag.errors.tip : null}
                                className="form-control"
                                placeholder={" Amount"}
                    ...
                    selectedViewDetail?.serviceType === "62d53abebf652aa3778946cd" ? (
                      <>
                        <div className="row my-3">
                          ...
                          <div className="col-md-6">
                            <label>
                              Total Driver Fare <span style={{ color: "#85BB65", fontWeight: "bold" }}>(in USD)</span>:
                            </label>
                            <Field name="driverBookingPrice">
                              {({ field }) => (
                                <Input
                        ...
                        <div className="row my-3">
                          <div className="col-md-6">
                            <label>
                              Driver 1 Fare <span style={{ color: "#85BB65", fontWeight: "bold" }}>(in USD)</span>:
                            </label>
                            <Field name="driverPrice">
                              {({ field }) => (
                                <Input
                                  {...field}
                                  type="number"
                          ...
                          <div className="col-md-6">
                            <label>
                              Driver 2 Fare <span style={{ color: "#85BB65", fontWeight: "bold" }}>(in USD)</span>:
                            </label>
                            <Field name="pairDriverPrice">
                              {({ field }) => (
                                <Input
                                  {...field}
                                  type="number"
                    ...
                    {selectedViewDetail?.status == "cancel" &&
                    selectedViewDetail?.serviceType !== "62d53abebf652aa3778946cd" ? (
                      <div className="row my-3">
                        <div className="col-md-6">
                          <label>
                            Admin Cancellation Commission <span style={{ color: "#85BB65", fontWeight: "bold" }}>(in USD)</span>
                            :
                          </label>
                          <Field name="rideAdminCommission">
                            {({ field }) => (
                        ...
                        <div className="col-md-6">
                          <label>
                            Driver Cancellation Fare <span style={{ color: "#85BB65", fontWeight: "bold" }}>(in USD)</span>:
                          </label>
                          <Field name="driverBookingPrice">
                            {({ field }) => (
                              <Input
                    ...
                    selectedViewDetail?.serviceType === "62d53abebf652aa3778946cd" ? (
                      <>
                        ...
                        <div className="row my-3">
                          <div className="col-md-6">
                            <label>
                              Driver 1 Cancellation Fare <span style={{ color: "#85BB65", fontWeight: "bold" }}>(in USD)</span>:
                            </label>
                            <Field name="driverPrice">
                              {({ field }) => (
                                <Input
                          ...
                          <div className="col-md-6">
                            <label>
                              Driver 2 Cancellation Fare <span style={{ color: "#85BB65", fontWeight: "bold" }}>(in USD)</span>:
                            </label>
                            <Field name="pairDriverPrice">
                              {({ field }) => (
                                <Input
                    ...
                    <div className="row my-3">
                      <div className="col-md-6">
                        <label>
                          Driver Tip<span style={{ color: "#85BB65", fontWeight: "bold" }}>(in USD)</span>:
                        </label>
                        <div style={{ fontWeight: "bold" }}>{selectedViewDetail?.driverTip == 0 ? "No" : "Yes"}</div>
                      </div>
                      {selectedViewDetail?.driverTip == 0 ? (
                        false
                      ) : (
                        <div className="col-md-6">
                          <div></div>
                          <Field name="driverTip">
                            {({ field }) => (
                              <Input
                                {...field}
                                type="number"
