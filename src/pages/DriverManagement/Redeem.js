...
export default function Redeem(props) {
  ...
  const handleChangePage = (event, newPage) => {
    ...
    getCategoriesContent(newPage + 1, rowsPerPage, ServiceTypeId, ActiveInactiveDuty, "approved", search);
    setPage(newPage);
  };
  ...
  const handleChangeRowsPerPage = (event) => {
    ...
    // setPage(0);
    getCategoriesContent(page, event.target.value, ServiceTypeId, ActiveInactiveDuty, "approved", search);
  ...
  const getCategoriesContent = async () => {
    try {
      const { data } = await axios.get(`/private/balanceTransactionDriver?id=${state[0]}&page=${page}&limit=${limit}`);

      console.log(data);
      setTableData([...data.result.docs]);
      setSearchedData(data.result.docs);
      settotalUserListCount(data.result.totalDocs);
      setStatus(status);
      setSearch(search);
      setDisplayNoDataImage(false);
      if (data.result.docs.length === 0 || data.status === 500) {
  ...
  //   try {
  //     const { data } = axios.get(`/private/balanceTransactionDriver?id=637484f45dbc07996a08da46&page=1&limit=10`);
  //     cosnole.log(data);
  //   } catch (error) {
  //     console.log(error);
  //   }
                    ...
                    <Table className={classes.table} stickyHeader>
                      <TableHead>
                        <TableRow>
                          ...
                          <TableCell style={{ textAlign: "center", fontWeight: "bold" }}>Bank Name</TableCell>
                          <TableCell style={{ textAlign: "center", fontWeight: "bold" }}> Amount</TableCell>
                          ...
                          tableData
                            ...
                            .map((category, index) => (
                              <TableRow hover key={index}>
                                ...
                                <TableCell style={{ textAlign: "center" }}>{category?.bank_name}</TableCell>
                                <TableCell className={classes.textMiddle} style={{ textAlign: "center" }}>
                                  {sanitizeDecimal(category?.amount)}
                                </TableCell>
                                <TableCell className={classes.textMiddle} style={{ textAlign: "center" }}>
